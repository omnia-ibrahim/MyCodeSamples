...
<div class="album_thumb">
  <?php 
    ...
    if ($img) {
     print l( $img.'<br/><span>'. $album->title . '</span>', 'node/'. $album->nid, array('html' => TRUE));
    }
    else {
     print l( theme('image', $image->images['_original'], $album->title, $album->title, array('width' => 128, 'height' => 128), FALSE).'<br/><span>'. $album->title . '</span>', 'node/'. $album->nid, array('html' => TRUE));   
    } 
  ?>
</div>
<?php
